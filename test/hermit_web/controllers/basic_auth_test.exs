defmodule HermitWeb.BasicAuthTest do
  ...
  setup do
    ...
    on_exit(fn ->
      if orig_user,
        do: System.put_env("HERMIT_BASIC_AUTH_USER", orig_user),
        else: System.delete_env("HERMIT_BASIC_AUTH_USER")

      if orig_pass,
        do: System.put_env("HERMIT_BASIC_AUTH_PASS", orig_pass),
        else: System.delete_env("HERMIT_BASIC_AUTH_PASS")
    end)
  ...
  test "when basic auth env vars are set, it allows access with correct credentials", %{
    conn: conn
  } do
    System.put_env("HERMIT_BASIC_AUTH_USER", "admin")
    System.put_env("HERMIT_BASIC_AUTH_PASS", "password")
  ...
  test "when basic auth env vars are set, it blocks access with incorrect credentials", %{
    conn: conn
  } do
    System.put_env("HERMIT_BASIC_AUTH_USER", "admin")
    System.put_env("HERMIT_BASIC_AUTH_PASS", "password")
